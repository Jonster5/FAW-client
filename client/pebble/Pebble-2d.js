    ...
    const obj = {
      ...
      scaleToWindow (bgcolor = 'grey', pointer) {
        ...
        if (parseFloat(this.domElement.style.width) > parseFloat(this.domElement.style.height)) {
          const margin = (window.innerHeight - parseFloat(this.domElement.style.height) * scaleX) / 2

          this.domElement.style.marginTop = margin + 'px'
          this.domElement.style.marginBottom = margin + 'px'
        } else {
          const margin = (window.innerWidth - parseFloat(this.domElement.style.width) * scaleY) / 2
      ...
      exportAsPNG (fileName = '') {
        try {
          const canvasElement = this.domElement
...
Pebble.getLagOffset = function (timestamp, func) {
  const fps = this.interpolationData._fps
...
Pebble.Rectangle = function (width = 32, height = 32, fillStyle = 'gray', strokeStyle = 'none', lineWidth = 0, x = 0, y = 0) {
  const sprite = new RectangleCanvasObject(width, height, fillStyle, strokeStyle, lineWidth, x, y)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Circle = function (diameter = 32, fillStyle = 'gray', strokeStyle = 'none', lineWidth = 0, x = 0, y = 0) {
  const sprite = new CircleCanvasObject(diameter, fillStyle, strokeStyle, lineWidth, x, y)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Line = function (ax = 0, ay = 0, bx = 0, by = 0, strokeStyle = 'none', lineWidth = 1) {
  const sprite = new LineCanvasObject(strokeStyle, lineWidth, ax, ay, bx, by)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Text = function (content = '', font = '12px sans-serif', fillStyle = 'black', x = 0, y = 0) {
  const sprite = new TextCanvasObject(content, font, fillStyle, x, y)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Group = function (...spritesToGroup) {
  const sprite = new ObjectGrouper(spritesToGroup)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Sprite = function (source, x = 0, y = 0) {
  const sprite = new SpriteCanvasObject(source, x, y)
  if (sprite.frames.length > 0) Pebble.addStatePlayer(sprite)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Button = function (source, x, y) {
  const sprite = new ButtonObject(source, x, y)
  if (stage) stage.addChild(sprite)
  return sprite
}
Pebble.Marker = function (x = 0, y = 0) {
  const sprite = new MarkerCanvasObject(x, y)
  if (stage) stage.addChild(sprite)
  return sprite
}
